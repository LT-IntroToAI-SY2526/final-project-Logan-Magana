...
# Chibi proportions: head pops out most, torso is a clearly shallower block, limbs thin
REGION_MAX_DEPTH: dict[int, int] = {
    REGION_HEAD:  14,   # prominent — biggest block
    REGION_TORSO:  9,   # clearly shallower than head → visible step at neck
    REGION_LIMB:   5,   # thin cylinders
...
MIN_PROFILE    = 0.30 # ellipse floor — tapers to 30% at edge for pronounced rounding
                      # (was 0.62 — that made everything a uniform cylinder)
SEAM_SCALE     = 0.50 # depth multiplier for rows at a region boundary
                      # creates a visible groove/indent between head, torso, limbs
...
def compute_depth_map(
    ...
    Phase A  Interior (bright) pixels get depth from region + elliptical profile.
             MIN_PROFILE = 0.30 so edges taper to 30% — pronounced rounded shape.
             Rows at a region boundary (seam) get SEAM_SCALE applied so there's
             a visible groove/indent between head, torso, and limbs.
    ...
    # Pre-compute per-row dominant region (majority vote among interior pixels)
    row_region = np.full(H, REGION_LIMB, dtype=np.int8)
    for y in range(H):
        xs = np.where(interior[y, :])[0]
        if len(xs) == 0:
            continue
        counts = np.bincount(region_map[y, xs].astype(np.int64), minlength=4)
        row_region[y] = int(counts.argmax())

    # Mark seam rows: where the dominant region changes from the row above
    is_seam_row = np.zeros(H, dtype=bool)
    for y in range(1, H):
        if (interior[y, :].any() and interior[y-1, :].any()
                and row_region[y] != row_region[y - 1]):
            is_seam_row[y]     = True
            is_seam_row[y - 1] = True   # both sides of the boundary get the groove
    ...
    for y in range(H):
        ...
        seam_mult    = SEAM_SCALE if is_seam_row[y] else 1.0
        ...
        for x in xs:
            ...
            raw             = max(max_d * MIN_PROFILE, min(profile * max_d, float(MAX_DEPTH)))
            depth_map[y, x] = raw * seam_mult
...
def smooth_grid(grid: VoxelGrid, passes: int = 2) -> VoxelGrid:
    ...
    but does NOT blur across region boundaries so seam grooves are preserved.

    Uses a bilateral-style approach: neighbours in the same region get full
    weight; neighbours in a different region get zero weight. This keeps the
    head/torso/limb depth steps sharp while still smoothing within each block.
    """
    H, W  = grid.H, grid.W
    depth = get_depth_map(grid).astype(np.float32)
    rmap  = grid.region_map   # (H, W) int8
                ...
                for ky in range(-1, 2):
                    for kx in range(-1, 2):
                        ...
                        if 0 <= ny < H and 0 <= nx2 < W and depth[ny, nx2] > 0:
                            # Only blend with same-region neighbours
