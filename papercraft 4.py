...
def prepare_back_face(sprite, size):
    """
    Generate a plausible back-of-head face with no facial features.

    Heuristic: for each row of the sprite, find the dominant bright color
    (ignoring dark outline/feature pixels below DARK_THRESHOLD). Fill that
    entire row on the back face with that color. This gives horizontal color
    bands matching the character's region colors — hat on top, hair below,
    shirt/body further down — without any eyes, mouth, or details bleeding through.

    Rows that are entirely dark (e.g. the top/bottom outline row) borrow their
    color from the nearest non-dark row above or below, so no black bands appear.
    """
    import numpy as np
    ...
    # Per-row dominant bright color; None for empty or all-dark rows
    row_colors = []
    for y in range(H):
        row_occ = occ[y, :]
        if not row_occ.any():
            ...
            continue
        row_cols = colors[y, row_occ]
        bright   = row_cols[row_cols.max(axis=1) >= DARK_THRESHOLD]
        if len(bright) == 0:
            row_colors.append(None)   # all-dark row (e.g. outline) — fill from neighbor
        else:
            row_colors.append(tuple(int(v) for v in np.median(bright, axis=0)))

    # Forward pass: fill None entries from the row above
    last = bg_rgb
    ...
    # Backward pass: fix any leading Nones that forward pass left as bg
    last = bg_rgb
    ...
    # Paint horizontal bands scaled to match front face centering
    face  = Image.new("RGB", (size, size), bg_rgb)
    draw  = ImageDraw.Draw(face)
    scale = min(size / W, size / H)
    disp_h, disp_w = int(H * scale), int(W * scale)
    oy, ox = (size - disp_h) // 2, (size - disp_w) // 2

    for y, col in enumerate(row_colors):
        y0 = oy + int(y * scale)
        y1 = oy + int((y + 1) * scale)
        if y1 > y0:
            draw.rectangle([ox, y0, ox + disp_w, y1], fill=col)
